...
final class TempoDomainTests: XCTestCase {
    ...
    func testGuidedSessionNeedsRecoveryBeforeResume() { var s = GuidedSessionMachine(); s.start(); s.beginActive(); s.rising(level: 7, threshold: 7); s.pause(); s.recovered(level: 5); XCTAssertEqual(s.state, .pausedRecovery); s.recovered(level: 4); XCTAssertEqual(s.state, .resumeReady) }
